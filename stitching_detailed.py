    ...
    for name in img_names:
        ...
        if work_megapix < 0:
            ...
            if is_work_scale_set is False:
                work_scale = min(1.0, np.sqrt(work_megapix * 1e6 / (imgObj.image.shape[0] * imgObj.image.shape[1])))
                is_work_scale_set = True
            img = cv.resize(src=imgObj.image, dsize=None, fx=work_scale, fy=work_scale, interpolation=cv.INTER_LINEAR_EXACT)
        if is_seam_scale_set is False:
            if seam_megapix > 0:
                seam_scale = min(1.0, np.sqrt(seam_megapix * 1e6 / (imgObj.image.shape[0] * imgObj.image.shape[1])))
            else:
                seam_scale = 1.0
